...
int main(){
	...
	for(int i = 0; i < t; i++){
		...
		closest.clear();
		closest.resize(numcomponents, vector<int>(numcomponents, n));
		mostrecent.clear();
		...
		for(int j = 0; j < n; j++){
			...
			for(int k = 0; k < numcomponents; k++){
				...
				if(mostrecent[k] >= 0 && k != nodeComponent[j]){
					...
					//cerr << "(dist: " << dist << ") ";
					if(dist < closest[nodeComponent[j]][k]){
						closest[nodeComponent[j]][k] = dist;
						closest[k][nodeComponent[j]] = dist;
		...
		if(nodeComponent[n-1] == 0){
			...
			leastCost = pow(closest[0][nodeComponent[n-1]], 2);
			//cerr << leastCost lin;
			for(int j = 1; j < numcomponents; j++){
				if(j != nodeComponent[n-1]){
					costPair = pow(closest[0][j], 2) + pow(closest[j][nodeComponent[n-1]], 2);
